...
type GoProgress = {moves:Move[];humanSide:Side};
...
export default function GoGame(){
  const [restored]=useState(()=>loadGameProgress<GoProgress>("go"));
  const [moves,setMoves]=useState<Move[]>(restored?.moves??[]);
  ...
  useEffect(()=>{touchGame("go")},[]);
  useEffect(()=>{if(moves.length&&!over)saveGameProgress<GoProgress>("go",{moves,humanSide})},[humanSide,moves,over]);
  useEffect(()=>{if(!over||!score)return;const key=`${score.black}:${score.white}:${moves.length}`;if(resultRef.current===key)return;resultRef.current=key;const winner=score.black===score.white?null:score.black>score.white?"black":"white",result=winner===null?"draw":winner===humanColor?"win":"loss";finishGame("go",result);playNovaSound(result==="win"?"success":result==="loss"?"error":"move")},[humanColor,moves.length,over,score]);
  useEffect(()=>subscribeGameReset("go",()=>restart(humanSide)),[humanSide]);
  ...
  const pass=()=>{if(over||thinking||turn!==humanColor)return;setMoves((current)=>[...current,null]);playNovaSound("move")};
  function restart(side=humanSide){requestIdRef.current++;resultRef.current="";clearGameProgress("go");touchGame("go");setHumanSide(side);setMoves([]);setResultDismissed(false);setThinking(false);setError("")}
  ...
  return <main className="go-game">
    ...
    {over&&score&&!resultDismissed&&<GameResultDialog tone={score.black===score.white?"draw":(score.black>score.white?"black":"white")===humanColor?"win":"loss"} title={score.black===score.white?"本局和棋":(score.black>score.white?"black":"white")===humanColor?"你赢了":"AI 获胜"} detail={`黑 ${score.black.toFixed(1)} · 白 ${score.white.toFixed(1)}`} onDismiss={()=>setResultDismissed(true)} onRestart={()=>restart()}/>}
  </main>
}
