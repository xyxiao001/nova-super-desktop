...
import { DragEvent as ReactDragEvent, MouseEvent as ReactMouseEvent, PointerEvent as ReactPointerEvent, TouchEvent as ReactTouchEvent, useCallback, useEffect, useLayoutEffect, useMemo, useReducer, useRef, useState } from "react";
...
const WINDOW_MINIMUMS:Partial<Record<WindowAppId,{width:number;height:number}>>={
  calendar:{width:700,height:520},
  settings:{width:560,height:430},
};
...
const fitAppWindowGeometry=(app:WindowAppId,geometry:WindowGeometry):WindowGeometry=>{const minimum=WINDOW_MINIMUMS[app];return fitWindowGeometry(geometry,window.innerWidth,window.innerHeight,minimum?.width,minimum?.height)};
const readWindowGeometry=(app:WindowAppId)=>{
  if(isCompactDesktopViewport())return null;
  const key=`${WINDOW_GEOMETRY_PREFIX}${app}`,saved=localStorage.getItem(key);
  if(!saved)return null;
  try{
    const geometry=JSON.parse(saved) as WindowGeometry,minimum=WINDOW_MINIMUMS[app];
    if(
      ![geometry.x,geometry.y,geometry.width,geometry.height].every(Number.isFinite)
      || (minimum&&(geometry.width<minimum.width||geometry.height<minimum.height))
    ){
...
export default function Home() {
  ...
  return <AppRuntimeProvider value={appRuntime}><main className={`super-desktop windows-desktop wallpaper-${settings.wallpaper} ${darkTheme?"theme-dark":"theme-light"} ${taskbarAutoHide?"taskbar-auto-hide":""} ${taskbarRevealed?"taskbar-revealed":""} ${mobileWindowOpen?"mobile-window-open":""} ${startOpen?"start-menu-open":""} ${systemPanelOpen?"system-panel-open":""}`} onPointerMove={(event)=>{updateMobileSearchPull(event);if(!taskbarAutoHide)return;const target=event.target as HTMLElement,reveal=event.clientY>=event.currentTarget.clientHeight-10||!!target.closest(".windows-taskbar,.start-menu,.taskbar-window-menu,.system-panel");if(reveal!==taskbarRevealed)setTaskbarRevealed(reveal)}} onPointerDown={(event)=>{beginMobileSearchPull(event);const target=event.target as HTMLElement;if(!target.closest(".desktop-item,.desktop-shortcut,.rename-dialog,.file-operation-dialog"))setSelectedIds([]);if(!target.closest(".desktop-menu"))setContextMenu(null);if(!target.closest(".taskbar-window-menu,.taskbar-entry"))setTaskbarMenu(null);if(!target.closest(".taskbar-entry"))setTaskbarPreview(null);if(!target.closest(".system-panel,.taskbar-clock"))setSystemPanelOpen(false);if(!target.closest(".start-menu,.start-button")){setStartOpen(false);setSearchQuery("")}}} onPointerUp={endMobileSearchPull} onPointerCancel={endMobileSearchPull} onTouchStart={beginMobileSearchTouch} onTouchMove={updateMobileSearchTouch} onTouchEnd={endMobileSearchTouch} onTouchCancel={endMobileSearchTouch} onContextMenu={(event)=>{if((event.target as HTMLElement).closest(".desktop-window,.windows-taskbar,.desktop-item,.desktop-shortcut"))return;event.preventDefault();setContextMenu({x:Math.min(event.clientX,window.innerWidth-225),y:Math.min(event.clientY,window.innerHeight-250)});focusDesktop();setStartOpen(false)}} onDragOver={(event)=>{if((event.target as HTMLElement).closest(".desktop-window"))return;const internal=event.dataTransfer.types.includes(NOVA_FILE_DRAG_TYPE);event.preventDefault();event.dataTransfer.dropEffect=internal?(event.ctrlKey||event.metaKey?"copy":"move"):"copy";setDraggingFiles(!internal)}} onDragLeave={(event)=>{if(!event.currentTarget.contains(event.relatedTarget as Node))setDraggingFiles(false)}} onDrop={(event)=>{if((event.target as HTMLElement).closest(".desktop-window"))return;event.preventDefault();setDraggingFiles(false);desktopDragIconRef.current=null;const ids=readDesktopDragIds(event.dataTransfer);if(ids.length){const alreadyOnDesktop=ids.every((id)=>items.find((item)=>item.id===id)?.parentId===null);if(!alreadyOnDesktop||event.ctrlKey||event.metaKey)requestFileOperation(event.ctrlKey||event.metaKey?"copy":"move",ids,null)}else void importFiles(event.dataTransfer.files)}}>
    ...
    {windowStates.calendar.open&&<AppWindow {...windowProps("calendar")}><LazyCalendarApp/></AppWindow>}
...
function AppWindow({app,title,icon,minimized,maximized,snapMode,focused,zIndex,onFocus,onClose,onMinimize,onMaximize,onSnap,onUnsnap,children}:{app:WindowAppId;title:string;icon:string;minimized:boolean;maximized:boolean;snapMode?:WindowSnapMode;focused:boolean;zIndex:number;onFocus:()=>void;onClose:()=>void;onMinimize:()=>void;onMaximize:()=>void;onSnap:(mode:WindowSnapMode)=>void;onUnsnap:()=>void;children:React.ReactNode}){
  ...
  useLayoutEffect(()=>{const element=windowRef.current;if(!element||isCompactDesktopViewport())return;const rect=element.getBoundingClientRect();setGeometry((current)=>current?fitAppWindowGeometry(app,current):fitAppWindowGeometry(app,{x:(window.innerWidth-rect.width)/2,y:(window.innerHeight-49-rect.height)/2,width:rect.width,height:rect.height}))},[app]);
  useEffect(()=>{if(snapMode&&!isCompactDesktopViewport())setGeometry(snappedWindowGeometry(snapMode,window.innerWidth,window.innerHeight))},[snapMode]);
  useEffect(()=>{if(geometry&&!isCompactDesktopViewport())localStorage.setItem(`${WINDOW_GEOMETRY_PREFIX}${app}`,JSON.stringify(geometry))},[app,geometry]);
  useEffect(()=>{const element=windowRef.current;if(!element||minimized||maximized||isCompactDesktopViewport())return;const observer=new ResizeObserver(()=>{const rect=element.getBoundingClientRect(),isMinimized=element.classList.contains("minimized"),isMaximized=element.classList.contains("maximized");if(!canMeasureWindowGeometry(isMinimized,isMaximized,rect.width,rect.height))return;setGeometry((current)=>{if(!current)return current;const width=Math.round(rect.width),height=Math.round(rect.height);return width===Math.round(current.width)&&height===Math.round(current.height)?current:fitAppWindowGeometry(app,{...current,width,height})})});observer.observe(element);return()=>observer.disconnect()},[app,maximized,minimized]);
  useEffect(()=>{const resize=()=>{if(isCompactDesktopViewport())return;setGeometry((current)=>snapMode?snappedWindowGeometry(snapMode,window.innerWidth,window.innerHeight):current?fitAppWindowGeometry(app,current):current)};window.addEventListener("resize",resize);return()=>window.removeEventListener("resize",resize)},[app,snapMode]);
  const startDrag=(event:ReactPointerEvent<HTMLDivElement>)=>{if(event.button!==0||maximized||isCompactDesktopViewport()||(event.target as HTMLElement).closest(".window-controls"))return;const element=windowRef.current;if(!element)return;event.preventDefault();onFocus();const rect=element.getBoundingClientRect();drag.current={startX:event.clientX,startY:event.clientY,originX:rect.left,originY:rect.top,moved:false};event.currentTarget.setPointerCapture(event.pointerId)};
  ...
  const startResize=(event:ReactPointerEvent<HTMLButtonElement>)=>{if(maximized||isCompactDesktopViewport()||event.button!==0)return;const rect=windowRef.current?.getBoundingClientRect();if(!rect)return;event.preventDefault();event.stopPropagation();onFocus();onUnsnap();resize.current={startX:event.clientX,startY:event.clientY,width:rect.width,height:rect.height};event.currentTarget.setPointerCapture(event.pointerId)};
  const moveResize=(event:ReactPointerEvent<HTMLButtonElement>)=>{const current=resize.current,element=windowRef.current;if(!current||!element)return;const rect=element.getBoundingClientRect(),minimum=WINDOW_MINIMUMS[app],width=clamp(current.width+event.clientX-current.startX,minimum?.width??320,Math.max(minimum?.width??320,window.innerWidth-rect.left-4)),height=clamp(current.height+event.clientY-current.startY,minimum?.height??260,Math.max(minimum?.height??260,window.innerHeight-49-rect.top));setGeometry((value)=>({x:value?.x??rect.left,y:value?.y??rect.top,width,height}))};
...
function useDesktopIconInteraction(id:string,position:IconPosition|undefined,move:((id:string,position:IconPosition)=>void)|undefined,onLongPress:(x:number,y:number)=>void,desktopPointerMove=true){
  ...
  const moved=useRef(false),longPressed=useRef(false);
  const clearPress=()=>{if(press.current)window.clearTimeout(press.current.timer);press.current=null};
  const triggerContextMenu=(x:number,y:number)=>{if(longPressed.current)return;clearPress();longPressed.current=true;onLongPress(x,y)};
  const start=(event:ReactPointerEvent<HTMLButtonElement>)=>{
    ...
    if(isCompactDesktopViewport()){
      ...
      if(position&&move){drag.current={x,y,origin:position};event.currentTarget.setPointerCapture(event.pointerId)}
      press.current={x,y,timer:window.setTimeout(()=>triggerContextMenu(x,y),DESKTOP_ICON_LONG_PRESS_MS)};
      return;
    }
  ...
  const update=(event:ReactPointerEvent<HTMLButtonElement>)=>{
    if(press.current){
      if(!movedBeyondLongPressTolerance(press.current,{x:event.clientX,y:event.clientY}))return;
      moved.current=true;
      clearPress();
    }
    ...
    if(isCompactDesktopViewport()){
      ...
      move(id,{x:event.clientX-parent.left,y:event.clientY-parent.top+scrollTop});
      return;
    }
    const dx=event.clientX-drag.current.x,dy=event.clientY-drag.current.y;
    if(Math.abs(dx)+Math.abs(dy)>3)moved.current=true;
    move(id,{x:clamp(drag.current.origin.x+dx,0,Math.max(0,parent.width-78)),y:clamp(drag.current.origin.y+dy,0,Math.max(0,parent.height-86))});
  };
  ...
  const contextMenu=(event:ReactMouseEvent<HTMLButtonElement>)=>{
    ...
    event.stopPropagation();
    if(isCompactDesktopViewport()){
      const x=press.current?.x??event.clientX,y=press.current?.y??event.clientY;
      triggerContextMenu(x,y);
    }else onLongPress(event.clientX,event.clientY);
  };
  const clickAction=(desktopAction:"ignore"|"select")=>{
    const action=desktopIconClickAction(isCompactDesktopViewport(),longPressed.current,moved.current,desktopAction);
    longPressed.current=false;
    return action;
  };
...
function DesktopShortcut({id,label,icon,kind,position,order,move,open,onIconDragEnter,onContextMenu}:{id:string;label:string;icon:string;kind:string;position:IconPosition;order:number;move:(id:string,position:IconPosition)=>void;open:()=>void;onIconDragEnter:(id:string)=>void;onContextMenu:(x:number,y:number)=>void}){const interaction=useDesktopIconInteraction(id,position,move,onContextMenu);return <button data-desktop-icon-id={id} className="desktop-shortcut positioned" style={{left:position.x,top:position.y,order}} onPointerDown={interaction.start} onPointerMove={interaction.update} onPointerUp={interaction.end} onPointerCancel={interaction.end} onDragEnter={()=>onIconDragEnter(id)} onClick={(event)=>{event.stopPropagation();if(interaction.clickAction("ignore")==="open")open()}} onDoubleClick={()=>{if(!isCompactDesktopViewport()&&!interaction.moved.current)open()}} onContextMenu={interaction.contextMenu} onKeyDown={(event)=>{if(event.key==="Enter")open()}}><span className={`shortcut-icon ${kind}-shortcut`}>{icon}</span><strong>{label}</strong></button>}
function DesktopFile({item,position,order,move,selected,cut=false,onSelect,onOpen,onDragStart,onDragEnd,onIconDragEnter,onFileDrop,onContextMenu}:{item:DesktopItem;position?:IconPosition;order:number;move?:(id:string,position:IconPosition)=>void;selected:boolean;cut?:boolean;onSelect:(event:ReactMouseEvent<HTMLButtonElement>)=>void;onOpen:()=>void;onDragStart?:(event:ReactDragEvent<HTMLButtonElement>)=>void;onDragEnd?:()=>void;onIconDragEnter?:(id:string)=>void;onFileDrop?:(event:ReactDragEvent<HTMLButtonElement>)=>void;onContextMenu?:(x:number,y:number)=>void}){const interaction=useDesktopIconInteraction(item.id,position,move,(x,y)=>onContextMenu?.(x,y),false);return <button draggable={!!onDragStart} data-desktop-icon-id={item.id} className={`desktop-item ${position?"positioned":""} ${selected?"selected":""} ${cut?"cut":""}`} style={position?{left:position.x,top:position.y,order}:undefined} onPointerDown={interaction.start} onPointerMove={interaction.update} onPointerUp={interaction.end} onPointerCancel={interaction.end} onDragStart={(event)=>{if(isCompactDesktopViewport()){event.preventDefault();return}onDragStart?.(event)}} onDragEnd={onDragEnd} onDragEnter={()=>onIconDragEnter?.(item.id)} onDragOver={(event)=>{if(!onFileDrop||!event.dataTransfer.types.includes(NOVA_FILE_DRAG_TYPE))return;event.preventDefault();event.stopPropagation();event.currentTarget.classList.add("drop-target")}} onDragLeave={(event)=>event.currentTarget.classList.remove("drop-target")} onDrop={(event)=>{event.currentTarget.classList.remove("drop-target");onFileDrop?.(event)}} onClick={(event)=>{event.stopPropagation();const action=interaction.clickAction("select");if(action==="open")onOpen();else if(action==="select")onSelect(event)}} onContextMenu={interaction.contextMenu} onDoubleClick={()=>{if(!isCompactDesktopViewport()&&!interaction.moved.current)onOpen()}} onKeyDown={(event)=>{if(event.key==="Enter")onOpen();if(event.shiftKey&&event.key==="F10"&&onContextMenu){event.preventDefault();const rect=event.currentTarget.getBoundingClientRect();onContextMenu(rect.left+20,rect.top+20)}}}>{item.type==="folder"?<span className="folder-icon"><i/></span>:item.type==="text"?<span className="text-icon"><b>TXT</b><i/><i/><i/></span>:<span className="image-icon" style={{backgroundImage:`url(${item.content})`}}/>}<strong>{item.name}</strong></button>}
