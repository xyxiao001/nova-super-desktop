...
export default function Home() {
  ...
  const [focused,setFocused]=useState<AppId>("desktop"),[clock,setClock]=useState(""),[contextMenu,setContextMenu]=useState<ContextMenuState|null>(null),[startOpen,setStartOpen]=useState(false),[searchQuery,setSearchQuery]=useState(""),[toast,setToast]=useState(""),[taskbarRevealed,setTaskbarRevealed]=useState(false);
  ...
  const openWindow=useCallback((app:WindowAppId)=>{updateWindow(app,{open:true,minimized:false});setFocused(app);setStartOpen(false);setContextMenu(null);setTaskbarRevealed(false)},[updateWindow]);
  const dismissWindow=useCallback((app:WindowAppId)=>updateWindow(app,{open:false}),[updateWindow]);
  const closeWindow=useCallback((app:WindowAppId)=>{dismissWindow(app);if(app==="photo")setPhotoSourceId(null);setFocused("desktop");setTaskbarRevealed(false)},[dismissWindow]);
  const minimizeWindow=useCallback((app:WindowAppId)=>{updateWindow(app,{minimized:true});setTaskbarRevealed(false)},[updateWindow]);
