...
export default function Home() {
  ...
  const beginMobileSearchTouch=(event:ReactTouchEvent<HTMLElement>)=>{
    ...
    if(!touch||!isCompactDesktopViewport()||mobileWindowOpen||startOpen||!desktop||desktop.scrollTop>0)return;
...
function useDesktopIconInteraction(id:string,position:IconPosition|undefined,move:((id:string,position:IconPosition)=>void)|undefined,onLongPress:(x:number,y:number)=>void){
  ...
  const update=(event:ReactPointerEvent<HTMLButtonElement>)=>{
    if(press.current){
      if(movedBeyondLongPressTolerance(press.current,{x:event.clientX,y:event.clientY})){moved.current=true;clearPress()}
      return;
    }
    if(!drag.current||!move)return;
