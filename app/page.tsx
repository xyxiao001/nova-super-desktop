...
const SETTINGS_SEARCH_ENTRIES=[
  {key:"settings:theme",label:"主题",detail:"设置 · 个性化",keywords:"明亮 深色 跟随系统"},
  {key:"settings:sound",label:"声音与音量",detail:"设置 · 声音",keywords:"静音 音效 音量"},
  {key:"settings:backup",label:"本地备份与恢复",detail:"设置 · 本地数据",keywords:"导入 导出 存储 数据"},
  {key:"settings:games",label:"清除游戏记录",detail:"设置 · 本地游戏数据",keywords:"存档 战绩 重置"},
];
...
export default function Home() {
  ...
  const createFolder=(parentId:string|null=null)=>{const item:DesktopItem={id:crypto.randomUUID(),type:"folder",name:uniqueName("新建文件夹"),content:"",parentId,createdAt:Date.now()};setFileUndo(null);setItems((current)=>[...current,item]);setSelectedIds(parentId?[]:[item.id]);setContextMenu(null)};
  const createText=(parentId:string|null=null)=>{const item:DesktopItem={id:crypto.randomUUID(),type:"text",name:uniqueName("未命名",".txt"),content:"",parentId,createdAt:Date.now()};setFileUndo(null);setItems((current)=>[...current,item]);setActiveNoteId(item.id);openWindow("notes")};
  ...
  const undoFileOperation=()=>{if(!fileUndo)return;setItems(fileUndo.items);setPositions(fileUndo.positions);setFileUndo(null);setPendingFileOperation(null);setSelectedIds([]);setToast(`已撤销：${fileUndo.label}`)};
  const restoreItem=(item:DesktopItem)=>{rememberFileUndo("还原项目");setItems(restoreDesktopItem(items,item.id));setToast(`${item.name} 已还原`)};
  ...
  const searchResults=[...searchApps.map((app)=>({key:`app:${app.key}`,label:app.label,icon:app.icon,detail:"应用",open:app.open})),...searchItems.map((item)=>({key:item.id,label:item.name,icon:item.type==="folder"?"▱":item.type==="image"?"▧":"TXT",detail:item.type==="folder"?"文件夹":item.type==="image"?"照片":"文本文稿",open:()=>openItem(item)})),...searchBooks.map((book)=>({key:`book:${book.id}`,label:book.title,icon:"阅",detail:`书籍 · ${book.author}`,open:()=>openWindow("reader")})),...searchSettings.map((entry)=>({key:entry.key,label:entry.label,icon:"⚙",detail:entry.detail,open:()=>openWindow("settings")}))];
  ...
  const finishRename=()=>{const name=renameValue.trim(),source=renameItemId?items.find((item)=>item.id===renameItemId):null;if(source&&name&&name!==source.name){rememberFileUndo("重命名项目");setItems(items.map((item)=>item.id===source.id?{...item,name}:item))}setRenameItemId(null)};
  ...
  return <main className={`super-desktop windows-desktop ${darkTheme?"theme-dark":"theme-light"} ${taskbarAutoHide?"taskbar-auto-hide":""} ${taskbarRevealed?"taskbar-revealed":""} ${startOpen?"start-menu-open":""}`} onPointerMove={(event)=>{if(!taskbarAutoHide)return;const target=event.target as HTMLElement,reveal=event.clientY>=event.currentTarget.clientHeight-10||!!target.closest(".windows-taskbar,.start-menu,.taskbar-window-menu");if(reveal!==taskbarRevealed)setTaskbarRevealed(reveal)}} onPointerDown={(event)=>{const target=event.target as HTMLElement;if(!target.closest(".desktop-item,.desktop-shortcut,.rename-dialog,.file-operation-dialog"))setSelectedIds([]);if(!target.closest(".desktop-menu"))setContextMenu(null);if(!target.closest(".taskbar-window-menu,.taskbar-entry"))setTaskbarMenu(null);if(!target.closest(".taskbar-entry"))setTaskbarPreview(null);if(!target.closest(".start-menu,.start-button")){setStartOpen(false);setSearchQuery("")}}} onContextMenu={(event)=>{if((event.target as HTMLElement).closest(".desktop-window,.windows-taskbar,.desktop-item,.desktop-shortcut"))return;event.preventDefault();setContextMenu({x:Math.min(event.clientX,window.innerWidth-225),y:Math.min(event.clientY,window.innerHeight-250)});setFocused("desktop");setStartOpen(false)}} onDragOver={(event)=>{if((event.target as HTMLElement).closest(".desktop-window"))return;const internal=event.dataTransfer.types.includes(NOVA_FILE_DRAG_TYPE);event.preventDefault();event.dataTransfer.dropEffect=internal?(event.ctrlKey||event.metaKey?"copy":"move"):"copy";setDraggingFiles(!internal)}} onDragLeave={(event)=>{if(!event.currentTarget.contains(event.relatedTarget as Node))setDraggingFiles(false)}} onDrop={(event)=>{if((event.target as HTMLElement).closest(".desktop-window"))return;event.preventDefault();setDraggingFiles(false);const ids=readDesktopDragIds(event.dataTransfer);if(ids.length)requestFileOperation(event.ctrlKey||event.metaKey?"copy":"move",ids,null);else void importFiles(event.dataTransfer.files)}}>
    ...
    {windowStates.explorer.open&&<AppWindow {...windowProps("explorer",activeFolder?.name??APP_REGISTRY.explorer.label)}><FileExplorer items={items} folderId={activeFolderId} clipboard={fileClipboard} canUndo={!!fileUndo} onNavigate={(folderId)=>{setActiveFolderId(folderId);if(folderId)updateItem(folderId,{lastOpenedAt:Date.now()})}} onOpen={openItem} onCreateFolder={createFolder} onCreateText={createText} onRename={beginRename} onSetClipboard={setClipboard} onPaste={pasteClipboard} onFileOperation={requestFileOperation} onTrash={(ids)=>{moveManyToRecycleBin(ids);focusWindow("explorer")}} onUndo={undoFileOperation} onOpenRecycle={()=>openWindow("recycle")}/></AppWindow>}
    ...
    {windowStates.settings.open&&<AppWindow {...windowProps("settings")}><SettingsApp settings={settings} onChange={updateSettings}/></AppWindow>}
