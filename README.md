...
NOVA 把文件管理、阅读、图片处理、笔记和本机游戏放进同一套桌面工作环境。应用和文件直接出现在桌面上，窗口可以移动、缩放、贴靠、最小化和恢复，任务栏负责管理正在运行的应用。
...
大型原生移植游戏及其移植脚本、资源与 skill 已迁至独立私有仓库，不包含在本仓库的构建和发布内容中。当前桌面不会加载这些外部游戏。

游戏和工具只在打开时加载。YouTD 2 等大型资源仍会占用首次下载流量与浏览器缓存空间，可以在设置中单独清理。
...
- Node.js `>= 22.13.0`
- npm
- Git LFS（用于获取 YouTD 2 的两个大型资源文件）
