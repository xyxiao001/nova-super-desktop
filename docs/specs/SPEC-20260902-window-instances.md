...
- [x] AC-14：TypeScript、相关单测、diff 审计及桌面与移动端最短浏览器关键路径均已通过。
...
- 关键交互：
  ...
  - 刷新后确认所有窗口关闭且用户数据保留。
- 浏览器验收结果：桌面端完成双资源管理器左右贴靠、双文稿与双图片独立打开、任务栏分组与实时预览、`Alt + Tab`、实例级最小化/恢复/关闭；移动端在 `390 × 844` 触控视口确认同类资源复用单一全屏实例。
- 不执行的验证及原因：按项目约定未运行 lint 或完整构建；浏览器验收复用已有本地开发服务，未额外启动开发服务器。
...
- 跨资源管理器窗口拖放已由 `SPEC-20260902-explorer-window-drag-drop.md` 完成；
...
- 2026-09-02：完成桌面与移动端浏览器关键路径，AC-14 通过，窗口实例化阶段正式收口。
