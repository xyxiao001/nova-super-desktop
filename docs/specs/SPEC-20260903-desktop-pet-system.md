...
- **用户掌控 AI**：AI 默认关闭；主动对话与 AI 主动陪伴分别授权，只有对应能力启用后才发送请求，且 Key 不进入普通备份、日志或 Service Worker 缓存。
- **克制存在**：宠物不能遮挡窗口操作，不能抢焦点，不能在没有明确用户命令时自动打开应用，也不能持续发声或弹出系统通知。
...
- 设置页可以控制宠物启用状态、动态强度、声音、桌面位置复位和本地数据重置。
...
- AI 主动陪伴使用独立开关，默认关闭；启用后只在已有本地空闲气泡上偶尔替换一句短台词。
...
1. 本机没有宠物数据时，系统自动创建名为 `Nova`、性格为“好奇”的默认宠物。
2. 宠物出现在桌面右下方安全区域，并用一次短台词说明它只在当前设备保存数据。
3. 用户可以直接互动，也可以稍后在“设置 → 桌面伙伴”修改名称、性格和行为偏好。
4. 首次进入不要求配置 AI，不展示登录或联网提示。
...
- 用户停留在桌面且一段时间没有输入时，宠物可以短暂躺卧、洗脸、泡澡、伸懒腰并显示本地自言自语。
- 主动陪伴只在桌面获得焦点且页面可见时计时；窗口聚焦、页面隐藏、专注进行中、宠物面板打开或用户恢复输入时不触发。
- 用户点击非最大化窗口外的空白桌面时，桌面必须恢复焦点并重新开始空闲计时。
- 主动陪伴间隔由气泡频率控制，本地姿态和基础台词不依赖 AI，也不写入对话记录和长期记忆。
- AI 主动陪伴开启后，单次输出最多 48 tokens，每个页面会话最多请求 2 次且至少间隔 30 分钟；用户恢复输入、桌面失焦或页面隐藏时立即取消。

### 本地对话与动作

- 未配置或未启用 AI 时，互动面板仍提供当前会话内的本地对话；首版覆盖问候、心情、精力、能力说明和常用应用意图。
- 本地对话只使用封闭规则生成回复，不保存原文，不请求网络，也不把用户输入写入宠物长期记忆。
- 应用动作使用白名单结构描述；“打开、启动、我想玩、我想看”等明确且低风险的本地命令在用户提交后直接通过 `WindowRuntime` 打开目标应用，建议型动作仍显示按钮。
- 本地系统命令先于 AI 解析，AI 是否启用、联网或成功响应都不影响基本应用操作；AI 返回文本本身不能触发系统动作。
- 命令注册表覆盖记事、阅读、照片、照片实验室、画板、专注、文件、日历、计算器、设置、回收站、游戏大厅和具体游戏；具体游戏命令优先于通用游戏大厅。
- 只有带明确操作语义的输入才执行；“五子棋真难”“这本书很好看”等普通讨论不打开应用。
...
| `activity` | 当前短时动作 | 待机、行走、阅读、画画、专注、庆祝、安慰、睡觉，以及本地短时躺卧、洗脸、泡澡、伸懒腰 |
...
| 专注时钟 | `focus-started` / `focus-ended` | 计时期间保持安静姿态，暂停、重置或关闭时结束 | 事件本身 |
| 专注时钟 | `focus-completed` | 安静陪伴后庆祝 | 完成时长档位 |
| 宠物本体 | `pet-interacted` | 摸摸时蹭手、击掌时庆祝、逗玩时扑球 | `pet/high-five/play` |
...
- “重置桌面伙伴”删除 `nova-pet` 和本地对话，随后立即创建新的默认宠物；不删除桌面文件、应用设置、AI 配置或游戏存档。
- 重置期间停止接收新的宠物活动，并在清库前等待已开始的运行时写入结束；重置前排队的旧状态不得写入新档案。
...
- “清除全部 AI 配置”删除全部连接档案、当前选中项和上下文权限；“重置桌面伙伴”不删除系统级 AI 配置，两个操作保持独立。
...
- 用户单独开启“AI 主动陪伴”后，前台桌面空闲气泡可以请求一句短台词；该请求不携带聊天历史、活动统计、资源名称或正文。
- 页面隐藏、应用未打开、离线或 AI 未启用时不发送请求，也不积累待发送队列。
- 请求失败、超时或返回格式错误时结束本次请求并显示本地状态；不自动重试、不自动切换模型或连接配置。
- 主动陪伴请求禁用网页搜索工具，超时 5 秒，输出最多 48 tokens；失败仍计入本次会话预算。
...
- 启用 AI 对话
- AI 主动陪伴：默认关闭，仅在启用 AI 对话且已选择连接时可开启
- 当前连接配置选择器
- 新增、编辑和删除连接配置
...
保存、新增、编辑、删除或切换配置均不触发自动测试请求；只有用户点击“测试当前配置”、主动开始 AI 对话，或明确开启 AI 主动陪伴并满足前台空闲预算时联网。
...
- `app/petAmbient.ts`：前台空闲间隔和本地自言自语/短时姿态选择。
- `app/petSystemCommands.ts`：不依赖 AI 的低风险系统命令注册表与匹配规则。
...
- `app/petProactiveAi.ts`：主动陪伴短提示词、token 上限、会话限次和冷却策略。
...
- [x] AC-1：本机没有宠物数据时自动创建默认宠物；刷新后名称、性格、状态和位置恢复。
- [x] AC-2：宠物关闭时不渲染动画层、不加载宠物大型资源，应用功能保持不变。
- [x] AC-3：宠物可以拖动和点击，可按自身宽度贴近桌面左右边缘，但不能遮挡任务栏、阻断文件拖放、抢夺窗口焦点或拦截桌面快捷键。
- [ ] AC-4：创作保存、专注完成、阅读里程碑、文件操作和游戏结果分别产生符合契约的活动事件。
  - [x] 照片/画板创作保存、阅读开始、阅读里程碑、专注开始/结束、专注完成和桌面文件操作已接入。
  - [ ] 游戏结果仍待后续迭代。
- [ ] AC-5：应用事件不包含正文、图片内容、完整棋盘或默认禁止的资源名称。
- [ ] AC-6：相同高频事件在冷却期内只更新必要统计，不连续播放气泡和长动画。
- [x] AC-7：页面隐藏或关闭期间不运行主动陪伴计时；再次打开只重新开始前台空闲计时，不惩罚用户、不补播事件。
- [x] AC-8：最大化窗口和移动端全屏应用打开时，宠物收起且不遮挡应用内容。
- [x] AC-9：减少动态效果启用时无行走位移或循环动画，宠物仍可通过静态姿态表达状态。
- [ ] AC-10：备份和恢复能够保留宠物档案与聚合记忆；重置桌面伙伴不影响其他应用数据。
...
- [x] AC-14：明确的低风险本地应用命令直接打开白名单应用；建议型动作仍需点击按钮，二者都发布不含对话文本的 `app-activated` 事件。
- [x] AC-15：`我想玩五子棋`、`我想看书`、`打开系统设置` 在 AI 开启或关闭时均由本地注册表稳定执行，普通讨论不会误启动应用。
- [x] AC-16：桌面前台空闲时按气泡频率轮换本地自言自语和短时姿态；任何用户输入都会重新计时，本地基础行为不依赖 AI。
...
- [x] AC-AI-12：流式回答期间可主动停止；停止后保留已接收文本，不自动重试、不追加错误回复。
- [x] AC-AI-13：AI 主动陪伴默认关闭；开启后单次最多 48 tokens、每会话最多 2 次、至少间隔 30 分钟，且不发送聊天历史、活动统计、资源名称或正文。
...
- 浏览器关键路径：首次默认创建、拖动、刷新、打开应用、完成一个事件、最大化窗口、切换移动端视口、备份恢复和重置伙伴。
...
- 2026-09-03：用户授权“真实陪伴联动”迭代；首批接入阅读器和专注时钟，全程只发布结构化本地事件，不自动请求 AI，也不采集阅读正文。
- 2026-09-03：用户继续授权创作保存反馈、流式停止生成和 Q 版动作深化；照片与画板只发布图片类型，直接互动只进入本地状态，不自动请求 AI。
- 2026-09-03：用户要求修复右侧拖拽范围并深化系统操作；本地白名单命令改为先于 AI 的确定性即时执行，AI 仍不获得系统调用权限。
- 2026-09-03：用户要求默认创建宠物并增强主动陪伴；新增仅前台桌面运行的空闲自言自语、四种生活姿态和文件操作反馈，重置后立即恢复默认伙伴。
- 2026-09-03：用户授权接入适度 AI 主动互动；采用独立授权、前台触发、每会话限次、长冷却、48-token 上限和禁用网页搜索的预算策略。
