...
import { DragEvent as ReactDragEvent, MouseEvent as ReactMouseEvent, PointerEvent as ReactPointerEvent, TouchEvent as ReactTouchEvent, Suspense, useCallback, useEffect, useMemo, useReducer, useRef, useState } from "react";
import { notifyWindowClosing, windowIsActive, WindowRuntimeProvider } from "../platform/windows/WindowRuntime";
import { textSearchResult } from "../../app/textSearch";
import StartMenu from "../../app/StartMenu";
import { sharedAppWindowState } from "../../app/standaloneAppRoute";
import {
  APP_REGISTRY,
  LAUNCHER_APPS,
  REGISTERED_APPS,
...
import DesktopObjectThemePicker from "./DesktopObjectThemePicker";
import {
...
import { PetRuntimeProvider } from "../platform/pet/PetRuntime";
import {
  WorkspaceRuntimeProvider,
  type WorkspacePhotoSource,
  type WorkspaceRuntimeValue,
} from "../platform/workspace/WorkspaceRuntime";
import "./futureDesktop.css";
import "./desktopThemes.css";
import { pairWorkspaceWindows } from "./futureWindowLayout";
import FutureWorkspace, { type FutureWorkspaceView } from "./FutureWorkspace";
...
const SETTINGS_SEARCH_ENTRIES=[
  ...
  {key:"settings:pet",sectionId:"pet",label:"桌面伙伴",detail:"设置 · 宠物与互动",keywords:"宠物 猫 心情 精力"},
  {key:"settings:ai",sectionId:"ai",label:"AI 连接",detail:"设置 · 本地连接与权限",keywords:"模型 API Key 请求地址"},
...
export default function DesktopRoot({ initialApp = null }: { initialApp?: WindowAppId | null }) {
  ...
  const [windowManager,dispatchWindow]=useReducer(windowInstanceReducer,initialApp,sharedAppWindowState),windowInstances=windowManager.instances,focused=windowManager.focused;
  ...
  const [futureView,setFutureView]=useState<FutureWorkspaceView>("apps"),[futureOverview,setFutureOverview]=useState(false);
  ...
  const [,refreshSaveStatus]=useReducer((revision:number)=>revision+1,0);
  ...
  const focusWindow=useCallback((id:WindowInstanceId)=>{setFutureOverview(false);dispatchWindow({type:"focus",id})},[]);
  const focusDesktop=useCallback(()=>dispatchWindow({type:"focus-desktop"}),[]);
  const closeWindowUi=useCallback(()=>{setFutureOverview(false);setStartOpen(false);setSystemPanelOpen(false);setContextMenu(null);setTaskbarMenu(null);setTaskbarPreview(null);setTaskbarRevealed(false)},[]);
  const openWindow=useCallback((app:WindowAppId)=>{const recent=instancesForApp(windowManager,app)[0],id=recent?.id??singletonWindowInstanceId(app);dispatchWindow({type:"open",id,app});publishNovaActivityEvent("app-activated",app);closeWindowUi();playNovaSound("open");return id},[closeWindowUi,windowManager]);
  const openNewWindow=useCallback((app:WindowAppId)=>{if(isCompactDesktopViewport()||APP_REGISTRY[app].window.instancePolicy!=="multiple")return openWindow(app);const id=createWindowInstanceId(app,crypto.randomUUID());dispatchWindow({type:"open",id,app});publishNovaActivityEvent("app-activated",app);closeWindowUi();playNovaSound("open");return id},[closeWindowUi,openWindow]);
  const openResourceWindow=useCallback((app:WindowAppId,target:WindowInstanceTarget)=>{const policy=APP_REGISTRY[app].window.instancePolicy,recent=instancesForApp(windowManager,app)[0],existing=policy==="per-resource"?selectResourceWindowInstance(windowManager,app,target,isCompactDesktopViewport()):recent;if(existing){dispatchWindow({type:"open",id:existing.id,app,target});publishNovaActivityEvent("app-activated",app);closeWindowUi();return existing.id}const id=policy==="per-resource"?createWindowInstanceId(app,crypto.randomUUID()):singletonWindowInstanceId(app);dispatchWindow({type:"open",id,app,target});publishNovaActivityEvent("app-activated",app);closeWindowUi();playNovaSound("open");return id},[closeWindowUi,windowManager]);
  ...
  const launchTarget=useCallback((target:AppLaunchTarget)=>{const resourceTarget=target.app==="notes"?{kind:"text" as const,itemId:target.itemId}:target.app==="explorer"&&target.parentId?{kind:"folder" as const,itemId:target.parentId}:undefined;const instanceId=resourceTarget?openResourceWindow(target.app,resourceTarget):openWindow(target.app);if(target.app==="explorer"&&!target.parentId)retargetWindow(instanceId);setLaunchIntents((current)=>({...current,[instanceId]:createAppLaunchIntent(++launchRequestRef.current,target)}))},[openResourceWindow,openWindow,retargetWindow]);
  ...
  const createFolder=(parentId:string|null=null)=>{const item:DesktopItem={id:crypto.randomUUID(),type:"folder",name:uniqueName("新建文件夹"),content:"",parentId,createdAt:Date.now()};rememberFileUndo("新建文件夹");setItems((current)=>[...current,item]);setSelectedIds(parentId?[]:[item.id]);setContextMenu(null);notifyFile(`${item.name} 已创建`,item.id);publishNovaActivityEvent("file-created","desktop",{itemType:"folder",count:1})};
  const createText=(parentId:string|null=null,sourceInstanceId?:WindowInstanceId)=>{const item:DesktopItem={id:crypto.randomUUID(),type:"text",name:uniqueName("未命名",".txt"),content:"",parentId,createdAt:Date.now()};rememberFileUndo("新建文稿");setItems((current)=>[...current,item]);notifyFile(`${item.name} 已创建`,item.id);const target:WindowInstanceTarget={kind:"text",itemId:item.id};if(sourceInstanceId)retargetWindow(sourceInstanceId,target);else openResourceWindow("notes",target);publishNovaActivityEvent("note-created","notes",{itemType:"text",count:1})};
  const createReaderExcerpt=({title,content,readerSource}:Parameters<WorkspaceRuntimeValue["createReaderExcerpt"]>[0])=>{const name=uniqueName(title,".txt"),item:DesktopItem={id:crypto.randomUUID(),type:"text",name,content,parentId:null,createdAt:Date.now(),...(readerSource?{readerSource}:{})};rememberFileUndo("创建阅读摘录");setItems((current)=>[...current,item]);notifyFile(`${name} 已保存到桌面`,item.id);openResourceWindow("notes",{kind:"text",itemId:item.id});publishNovaActivityEvent("excerpt-created","reader",{itemType:"text",count:1})};
  ...
  const moveManyToRecycleBin=(ids:string[])=>{const roots=topLevelDesktopItemIds(items,ids);if(!roots.length)return;const affected=descendantIds(items,roots);rememberFileUndo("移到回收站");setItems(trashDesktopItems(items,roots));closeAffected(affected);setSelectedIds([]);setContextMenu(null);focusDesktop();notifyFile(`${roots.length} 个项目已移到回收站`);publishNovaActivityEvent("files-organized","desktop",{operation:"trash",count:roots.length})};
  ...
  const restoreMany=(ids:string[])=>{const result=restoreDesktopItems(items,ids);if(!result.resultIds.length)return;rememberFileUndo("还原项目");setItems(result.items);notifyFile(`${result.resultIds.length} 个项目已还原`,result.resultIds.length===1?result.resultIds[0]:undefined);publishNovaActivityEvent("files-organized","desktop",{operation:"restore",count:result.resultIds.length})};
  const permanentlyDeleteMany=(ids:string[])=>{const result=permanentlyDeleteDesktopItems(items,ids);setFileUndo(null);setItems(result.items);setPositions((current)=>Object.fromEntries(Object.entries(current).filter(([id])=>!result.removedIds.has(id))));setDesktopObjects((current)=>removeDesktopObjects(current,result.removedIds));closeAffected(result.removedIds);setSelectedIds([]);setContextMenu(null);notifyFile(`${ids.length} 个项目已永久删除`);if(result.removedIds.size)publishNovaActivityEvent("files-organized","desktop",{operation:"delete",count:result.removedIds.size})};
  const emptyRecycleBin=()=>{const result=permanentlyDeleteDesktopItems(items,items.filter((item)=>item.deletedAt).map((item)=>item.id));setFileUndo(null);setItems(result.items);setPositions((current)=>Object.fromEntries(Object.entries(current).filter(([id])=>!result.removedIds.has(id))));setDesktopObjects((current)=>removeDesktopObjects(current,result.removedIds));closeAffected(result.removedIds);notifyFile("回收站已清空");if(result.removedIds.size)publishNovaActivityEvent("files-organized","desktop",{operation:"delete",count:result.removedIds.size})};
  ...
  const removeNote=(id:string)=>{rememberFileUndo("删除文稿");setItems((current)=>trashDesktopItems(current,[id]));closeAffected(new Set([id]));notifyFile("文稿已移到回收站");publishNovaActivityEvent("files-organized","desktop",{operation:"trash",count:1})};
  ...
  const searchText=searchQuery.trim().toLowerCase(),searchApps=searchText?appEntries.filter((app)=>`${app.label} ${app.kind}`.toLowerCase().includes(searchText)):[],searchItems=searchText?visibleItems.map((item)=>({item,match:item.type==="text"?textSearchResult(item.content,searchQuery):null})).filter(({item,match})=>item.name.toLowerCase().includes(searchText)||!!match):[],searchBooks=searchText?readerSearchBooks.filter((book)=>`${book.title} ${book.author}`.toLowerCase().includes(searchText)):[],searchSettings=searchText?SETTINGS_SEARCH_ENTRIES.filter((entry)=>`${entry.label} ${entry.detail} ${entry.keywords}`.toLowerCase().includes(searchText)):[];
  ...
  const searchResults=[...searchApps.map((app)=>({key:`app:${app.key}`,label:app.label,icon:app.icon,detail:"应用",open:app.open})),...searchItems.map(({item,match})=>({key:item.id,label:item.name,icon:item.type==="folder"?"▱":item.type==="image"?"▧":"TXT",detail:match?match.excerpt:`${item.type==="folder"?"文件夹":item.type==="image"?"照片":"文本文稿"} · ${item.type==="text"?"打开文稿":"打开所在位置"}`,open:()=>item.type==="text"?launchTarget({app:"notes",kind:"text",itemId:item.id,...(match?{selection:{start:match.start,end:match.end,query:searchQuery.trim()}}:{})}):launchTarget({app:"explorer",kind:"file",itemId:item.id,parentId:item.parentId})})),...searchBooks.map((book)=>({key:`book:${book.id}`,label:book.title,icon:"阅",detail:`书籍 · ${book.author}`,open:()=>launchTarget({app:"reader",kind:"book",bookId:book.id})})),...searchSettings.map((entry)=>({key:entry.key,label:entry.label,icon:"⚙",detail:entry.detail,open:()=>launchTarget({app:"settings",kind:"section",sectionId:entry.sectionId})})),...searchSystem];
  ...
  const resolvePositions=(source:Record<string,IconPosition>)=>desktopEntryIds.reduce<Record<string,IconPosition>>((result,id)=>{
    ...
    if(source[id]){result[id]=source[id];return result}
    let slot=0;
    while(overlaps(defaultPosition(slot)))slot++;
    result[id]=defaultPosition(slot);
    return result;
  },Object.fromEntries(desktopEntryIds.filter(id=>id in source).map(id=>[id,source[id]])));
  ...
  const dropDesktopFiles=(event:ReactDragEvent<HTMLElement>)=>{
    if((event.target as HTMLElement).closest(".desktop-window"))return;
    event.preventDefault();
    setDraggingFiles(false);
    const ids=readDesktopFileDragIds(event.dataTransfer);
    if(!ids.length){void importFiles(event.dataTransfer.files);return}
    const mode=desktopFileDragMode(event),alreadyOnDesktop=ids.every((id)=>items.find((item)=>item.id===id)?.parentId===null);
    if(alreadyOnDesktop&&mode==="move")return;
    requestFileOperation(mode,ids,null);
  };
  const dropDesktopFileOnFolder=(sourceId:string,x:number,y:number)=>{
    const targetId=document.elementsFromPoint(x,y).map((element)=>element.closest<HTMLElement>("[data-desktop-folder-id]")?.dataset.desktopFolderId).find((id)=>id&&id!==sourceId);
    if(!targetId)return;
    const ids=selectedIds.includes(sourceId)?selectedIds:[sourceId];
    requestFileOperation("move",ids,targetId);
  };
  ...
  const openItemMenu=(item:DesktopItem,x:number,y:number)=>{if(!selectedIds.includes(item.id))setSelectedIds([item.id]);setContextMenu({x:Math.min(x,window.innerWidth-225),y:Math.min(y,window.innerHeight-(desktopObjectIds.has(item.id)?450:350)),itemId:item.id});setStartOpen(false)};
  ...
  const workspaceRuntime:WorkspaceRuntimeValue={
    items,
    visibleItems,
    getSaveStatus:(item)=>storageState==="error"?"error":desktopSyncRef.current?.getItemStatus(item)??"saving",
    downloadItem,
    ...
    savePhotoEdit,
    createReaderExcerpt,
    openReaderSource:(source)=>launchTarget({app:"reader",kind:"book",bookId:source.bookId,source}),
  };
  ...
  useEffect(()=>{
    const shortcut=(event:KeyboardEvent)=>{
      ...
      if(event.ctrlKey&&event.code==="Space"){event.preventDefault();setFutureOverview(false);setStartMode("launcher");setStartOpen((current)=>!current);setSearchQuery("");setSearchIndex(0);return}
  ...
  return <PetRuntimeProvider><WindowRuntimeProvider value={windowRuntime}><LaunchRuntimeProvider value={launchRuntime}><SettingsRuntimeProvider value={settingsRuntime}><WorkspaceRuntimeProvider value={workspaceRuntime}><main className={`super-desktop windows-desktop desktop-style-${settings.desktopStyle} desktop-look-${settings.desktopLook} wallpaper-${settings.wallpaper} ${darkTheme?"theme-dark":"theme-light"} ${taskbarAutoHide?"taskbar-auto-hide":""} ${taskbarRevealed?"taskbar-revealed":""} ${mobileWindowOpen?"mobile-window-open":""} ${startOpen?"start-menu-open":""} ${systemPanelOpen?"system-panel-open":""}`} onPointerMove={(event)=>{if(settings.desktopStyle==="future"||settings.desktopLook!=="original"){event.currentTarget.style.setProperty("--space-x",`${(event.clientX/event.currentTarget.clientWidth-.5)*24}px`);event.currentTarget.style.setProperty("--space-y",`${(event.clientY/event.currentTarget.clientHeight-.5)*18}px`)}updateMobileSearchPull(event);if(!taskbarAutoHide)return;const target=event.target as HTMLElement,reveal=event.clientY>=event.currentTarget.clientHeight-10||!!target.closest(".windows-taskbar,.start-menu,.taskbar-window-menu,.system-panel");if(reveal!==taskbarRevealed)setTaskbarRevealed(reveal)}} onPointerDown={(event)=>{beginMobileSearchPull(event);const target=event.target as HTMLElement;if(!target.closest(".future-workspace,.future-overview-toggle")&&shouldFocusDesktopFromTarget(target))focusDesktop();if(!target.closest(".desktop-item,.desktop-shortcut,.desktop-creative-object,.rename-dialog,.file-operation-dialog"))setSelectedIds([]);if(!target.closest(".desktop-menu"))setContextMenu(null);if(!target.closest(".taskbar-window-menu,.taskbar-entry"))setTaskbarMenu(null);if(!target.closest(".taskbar-entry"))setTaskbarPreview(null);if(!target.closest(".system-panel,.taskbar-clock"))setSystemPanelOpen(false);if(!target.closest(".start-menu,.start-button")){setStartOpen(false);setSearchQuery("")}}} onPointerUp={endMobileSearchPull} onPointerCancel={endMobileSearchPull} onTouchStart={beginMobileSearchTouch} onTouchMove={updateMobileSearchTouch} onTouchEnd={endMobileSearchTouch} onTouchCancel={endMobileSearchTouch} onContextMenu={(event)=>{if((event.target as HTMLElement).closest(".desktop-window,.windows-taskbar,.desktop-item,.desktop-shortcut,.desktop-creative-object,.desktop-pet-layer"))return;event.preventDefault();setContextMenu({x:Math.min(event.clientX,window.innerWidth-225),y:Math.min(event.clientY,window.innerHeight-250)});focusDesktop();setStartOpen(false)}} onDragOver={(event)=>{if((event.target as HTMLElement).closest(".desktop-window"))return;const internal=hasDesktopFileDrag(event.dataTransfer);event.preventDefault();event.dataTransfer.dropEffect=internal?desktopFileDragMode(event):"copy";setDraggingFiles(!internal)}} onDragLeave={(event)=>{if(!event.currentTarget.contains(event.relatedTarget as Node))setDraggingFiles(false)}} onDrop={dropDesktopFiles}>
    ...
    {settings.desktopLook!=="original"&&<div className="desktop-theme-scene" aria-hidden="true"><i/><i/><i/></div>}
    {settings.desktopStyle==="future"&&<div className="future-space" aria-hidden="true"><div className="future-lattice"/><div className="future-orbit"><i/><i/><i/></div><div className="future-horizon"/><span>NOVA <b>SPACE</b></span></div>}
    {settings.desktopStyle==="future"&&<>
      <FutureWorkspace view={futureView} overview={futureOverview} instances={windowInstances} focused={focused}
        onViewChange={(view)=>{setFutureView(view);if(view==="windows")setFutureOverview(true);if(view==="files"){setFutureOverview(false);allWindowInstances(windowInstances).filter((instance)=>!instance.minimized).forEach((instance)=>minimizeWindow(instance.id));focusDesktop()}}}
        onDismiss={()=>setFutureOverview(false)}
        onSearch={()=>{setFutureOverview(false);setStartMode("search");setStartOpen(true);setSearchQuery("");setSearchIndex(0)}}
        onLaunch={launchApp} onFocus={focusWindow} onMinimize={minimizeWindow} onClose={closeWindow}
        onPair={(left,right)=>{pairWorkspaceWindows(left,right).forEach(dispatchWindow);setFutureOverview(false)}}/>
      <button className="future-overview-toggle" aria-label="空间总览" aria-expanded={futureOverview} onClick={()=>{setFutureView("windows");setFutureOverview(!futureOverview);setStartOpen(false);setSystemPanelOpen(false)}}><span aria-hidden="true">▱</span><span>空间总览</span></button>
    </>}
