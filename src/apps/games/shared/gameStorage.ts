export type GameId = "mines"|"chess"|"gomoku"|"tower"|"youtd2"|"wolfslot"|"frontline"|"fanren";
...
const GAME_AUXILIARY_KEYS=["nova-mines-difficulty","nova-mines-best","nova-game-coins",FRONTLINE_NATIVE_SAVE_KEY,FANREN_NATIVE_SAVE_KEY];
const GAME_IDS:GameId[]=["mines","chess","gomoku","tower","youtd2","wolfslot","frontline","fanren"];
...
export const readGameRecords=():GameRecords=>{
  ...
  try{
    ...
    for(const id of GAME_IDS)defaults[id]={...defaults[id],...saved[id],hasProgress:localStorage.getItem(id==="frontline"?FRONTLINE_NATIVE_SAVE_KEY:id==="fanren"?FANREN_NATIVE_SAVE_KEY:`${PROGRESS_PREFIX}${id}`)!==null};
