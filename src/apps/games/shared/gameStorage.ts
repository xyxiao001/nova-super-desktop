export type GameId = "mines"|"chess"|"gomoku"|"tower"|"youtd2"|"wolfslot"|"frontline"|"fanren"|"doupo";
...
const DOUPO_NATIVE_SAVE_KEY="nova-game-doupo-save";
const FANREN_NATIVE_SAVE_KEY="nova-fanren-local-v1";
...
const GAME_AUXILIARY_KEYS=["nova-mines-difficulty","nova-mines-best","nova-game-coins",FRONTLINE_NATIVE_SAVE_KEY,FANREN_NATIVE_SAVE_KEY,DOUPO_NATIVE_SAVE_KEY];
const GAME_IDS:GameId[]=["mines","chess","gomoku","tower","youtd2","wolfslot","frontline","fanren","doupo"];
...
export const readGameRecords=():GameRecords=>{
  ...
  try{
    ...
    for(const id of GAME_IDS)defaults[id]={...defaults[id],...saved[id],hasProgress:localStorage.getItem(id==="frontline"?FRONTLINE_NATIVE_SAVE_KEY:id==="fanren"?FANREN_NATIVE_SAVE_KEY:id==="doupo"?DOUPO_NATIVE_SAVE_KEY:`${PROGRESS_PREFIX}${id}`)!==null};
...
export const clearGameProgress=(id:GameId)=>{
  ...
  if(id==="doupo")localStorage.removeItem(DOUPO_NATIVE_SAVE_KEY);
  if(id==="fanren")localStorage.removeItem(FANREN_NATIVE_SAVE_KEY);
