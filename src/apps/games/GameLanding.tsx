...
export default function GameLanding({gameId,onStart}:{gameId:GameId;onStart:()=>void}) {
 ...
 return <main className="game-landing" aria-label={`${game.label}游戏介绍`}>
  ...
  <section className="game-landing-card">
   <div className="game-landing-art"><img className="game-landing-cover" src={game.artwork} alt={game.label}/><span className="game-art-caption">{game.category+' · 随时开局'}</span></div>
   <div className="game-landing-content"><p className="game-landing-category">{game.category} / 即点即玩</p><h1>{game.label}</h1><p className="game-landing-meta">{game.meta}</p><p className="game-landing-description">留一点时间给下一局。打开游戏，在熟悉的规则里迎接新的挑战。</p><div className="game-landing-facts"><span>全窗口游玩</span><span>本地存档</span><span>即开即玩</span></div><button onClick={onStart}>开始游戏 <span aria-hidden="true">↗</span></button><p className="game-landing-note">直接进入全窗口游戏 · 随时返回桌面<br/>进度保存在当前设备，分享链接不包含存档。</p></div>
  </section>
