...
function PhotoEditorWorkspace({active,initialImage,onSave}:{active:boolean;initialImage:WorkspacePhotoSource|null;onSave:(mode:"copy"|"replace",name:string,content:string)=>void}) {
  ...
  const exportImage=(saveMode:"copy"|"replace")=>{const canvas=canvasRef.current;if(!canvas)return;const{x,y,w,h}=edit.crop,sx=Math.round(x*canvas.width),sy=Math.round(y*canvas.height),sw=Math.max(1,Math.round(w*canvas.width)),sh=Math.max(1,Math.round(h*canvas.height)),output=document.createElement("canvas");output.width=sw;output.height=sh;output.getContext("2d")?.drawImage(canvas,sx,sy,sw,sh,0,0,sw,sh);onSave(saveMode,`${fileName}-已编辑.jpg`,output.toDataURL("image/jpeg",.9));if(saveMode==="copy"){playNovaSound("success");publishNovaSystemMoment("creative-save","photo")}if(saveMode==="replace"){setEdit(initialEdit);setPast([]);setFuture([]);setZoom(1);setPan({x:0,y:0})}setReplaceConfirm(false)};
